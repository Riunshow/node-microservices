...
export class RpcRegistry {
  ...
  private static _start() {
    ...
    this.server.bindAsync(address, this._credentials, (err) => {
      if (err) {
        console.log(`error: ${err}`) 
      } else {
        this.server.start()
        console.log(`rpc server 启动成功: ${address}`) 
      }
    })
  }
    ...
    for (let serviceContainer of ServiceContainer.services) {
      ...
      for (let key in serviceContainer.service) {
        ...
        if (route) {
          ...
          let _func = ServiceContainer.generateRouteFunc(serviceContainer.service, key, route)
          routeContainer.func = _func
          rpc[key] = routeContainer.func
        }
      }

      this.server.addService(serviceContainer.service, rpc as any)
    }
  }
